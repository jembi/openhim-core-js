...
This resource enables metrics about transaction to be extracts from the OpenHIM in a flexible way. There are various forms for this endpoint depoending on the format of the metrics that you want to get out. Metrics will only be returned for the channels that a the API user has access to.
...
There are a few diffferent fors of this endpoint that return metrics broken down in different ways:
...
The metrics API always returns a a JSON array, even if it is returning just one metrics object, with a `200` response. A `401` response will be returned if a specified channel doesn't exist. Each metrics object in the array has the following format:
