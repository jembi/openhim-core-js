...
let routerConf = config.get('router');
let utils = require('../../lib/utils');
let router = require('../../lib/middleware/router');

let invertPathTransform = pathTransform =>
  ...
  if (channel.addAutoRewriteRules) {
    ...
    return utils.getAllChannelsInPriorityOrder(function(err, channels) {
      ...
      for (channel of Array.from(channels)) {
        for (var route of Array.from(channel.routes)) {
          (function() {
            if (route.primary) {
              /*
...
let rewriteUrls = (body, channel, authType, callback) =>
  exports.fetchRewriteConfig(channel, authType, function(err, rwConfig) {
    if (err != null) {
      return callback(err);
    }
    ...
    let newBody = body.replace(/["|']?(?:href|src|fullUrl)["|']?[:|=]\s?["|'](\S*?)["|']/g, function(match, hrefUrl) {
      let relativePath;
      let hrefUrlObj = url.parse(hrefUrl);
      ...
      for (let rewriteRule of Array.from(rwConfig)) {
      ...
      let replacement = url.format(hrefUrlObj);
...
export function koaMiddleware(next) {
  ...
  if (this.authorisedChannel.rewriteUrls) {
    let rewrite = Q.denodeify(rewriteUrls);
