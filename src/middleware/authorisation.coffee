  ...
  Channel.find {}, (err, channels) ->
    for channel in channels
      ...
      if pat.test ctx.request.url
        ...
        if matchedRoles.length > 0 or (channel.allow.indexOf ctx.authenticated.clientID) isnt -1 or (channel.whitelist.indexOf ctx.ip) isnt -1 or (channel.type == 'public') is true
