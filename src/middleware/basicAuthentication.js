...
const bcryptCompare = (pass, client, callback) =>
  bcrypt.compare(pass, client.passwordHash, callback)
...
export function authenticateUser(ctx, done) {
  ...
  if (user) {
      ...
      if (client) {
        ...
        return comparePasswordWithClientHash(user.pass, client, (err, res) => {
          ...
          if (res) {
            ...
            logger.info(
              `${user.name} could NOT be authenticated, trying next auth mechanism if any...`
            )
            return done(null, null)
          }
        })
        ...
        logger.info(
          `${user.name} not found, trying next auth mechanism if any...`
        )
        return done(null, null)
      }
    })
    ...
    logger.info(
      'No basic auth details supplied, trying next auth mechanism if any...'
    )
...
export async function koaMiddleware(ctx, next) {
  if (ctx.authenticated != null) {
    ...
    if (
      (ctx.authenticated != null ? ctx.authenticated.clientID : undefined) !=
      null
    ) {
