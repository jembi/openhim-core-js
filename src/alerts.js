...
const trxURL = trx =>
  `${config.alerts.consoleURL}/#!/transactions/${trx.transactionID}`
...
The following transaction(s) have completed with status ${
      alert.status
...
`
    text += transactions
      .map(
        trx =>
          `        <tr><td><a href='${trxURL(trx)}'>${trxURL(
            trx
          )}</a></td></tr>`
      )
      .join('\n')
...
The following transaction(s) have been retried ${
      channel.autoRetryMaxAttempts
    } times, but are still failing:
...
function findOneAlert(channel, alert, dateFrom, user, alertStatus, callback) {
  const criteria = {
    ...
    status:
      alert.condition === 'auto-retry-max-attempted' ? '500' : alert.status,
    alertStatus
  }
  if (user) {
    criteria.user = user
  }
  return AlertModel.findOne(criteria).exec(callback)
}
...
function findTransactionsMatchingStatus(channel, alert, dateFrom, callback) {
  ...
  return findTransactions(channel, dateToCheck, statusMatch, (err, results) => {
    if (!err && results != null && alert.failureRate != null) {
      ...
      return countTotalTransactionsForChannel(
        channel,
        dateToCheck,
        (err, count) => {
          logger.debug(
            `.countTotalTransactionsForChannel: ${new Date() - _countStart} ms`
          )
...
function userAlreadyReceivedAlert(channel, alert, user, callback) {
  ...
  if (!dateFrom) {
    return callback(
      new Error(`Unsupported option 'maxAlerts=${user.maxAlerts}'`)
    )
  }
  ...
  return findOneAlert(
    ...
    user.user,
    'Completed',
    (err, userAlert) => callback(err != null ? err : null, !!userAlert)
  )
}
...
const sendAlert = (channel, alert, user, transactions, contactHandler, done) =>
  UserModel.findOne({email: user.user}, (err, dbUser) => {
    ...
    return userAlreadyReceivedAlert(channel, alert, user, (err, received) => {
      ...
      logger.info(
        `Sending alert for user '${user.user}' using method '${user.method}'`
      )

      return getTransactionsForAlert(
        channel,
        alert,
        user,
        transactions,
        (err, transactionsForAlert) => {
          if (err) {
            done(err)
          }
          ...
          if (user.method === 'email') {
            ...
            if (!dbUser.msisdn) {
              return done(
                `Cannot send alert: MSISDN not specified for user '${user.user}'`
              )
            }
          ...
          return done(
            `Unknown method '${user.method}' specified for user '${user.user}'`
          )
        }
      )
    })
...
function sendAlerts(channel, alert, transactions, contactHandler, done) {
  ...
  if (alert.groups) {
    for (const group of Array.from(alert.groups)) {
      const groupDefer = new Promise(resolve => {
...
function alertingTask(job, contactHandler, done) {
  ...
  const lastAlertDate =
    job.attrs.data.lastAlertDate != null
      ? job.attrs.data.lastAlertDate
      : new Date()
        ...
        for (const alert of Array.from(channel.alerts)) {
          ;(function (channel, alert) {
            const deferred = new Promise(resolve => {
              ...
              findTransactionsMatchingCondition(
                channel,
                alert,
                lastAlertDate,
                (err, results) => {
                  logger.debug(
                    `.findTransactionsMatchingStatus: ${
                      new Date() - _findStart
                    } ms`
                  )
...
export function setupAgenda(agenda) {
  agenda.define('generate transaction alerts', (job, done) =>
    alertingTask(job, contact.contactUser, done)
  )
  return agenda.every(
    `${config.alerts.pollPeriodMinutes} minutes`,
