...
function startListening(channel, tcpServer, host, port, callback) {
  ...
  return tcpServer.on('error', err =>
    logger.error(`${err} Host: ${host} Port: ${port}`)
  )
}

export function notifyMasterToStartTCPServer(channelID) {
...
export function startupTCPServer(channelID, callback) {
  ...
  return ChannelModel.findById(channelID, (err, channel) => {
    ...
    if (!port) {
      return callback(
        new Error(
          `Channel ${channel.name} (${channel._id}): TCP port not defined`
        )
      )
    }
    ...
    if (channel.type === 'tls') {
      return tlsAuthentication.getServerOptions(true, (err, options) => {
        ...
        return startListening(channel, tcpServer, host, port, err => {
          if (err) {
            ...
            logger.info(
              `Channel ${channel.name} (${channel._id}): TLS server listening on port ${port}`
            )
            return callback(null)
          }
        })
      ...
      return startListening(channel, tcpServer, host, port, err => {
        if (err) {
          ...
          logger.info(
            `Channel ${channel.name} (${channel._id}): TCP server listening on port ${port}`
          )
          return callback(null)
        }
      })
...
export function notifyMasterToStopTCPServer(channelID) {
