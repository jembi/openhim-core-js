...
function parseAuditRecordFromXML(xml, callback) {
  ...
  return parseString(xml, options, (err, result) => {
    ...
    if (result.auditMessage.auditSourceIdentification) {
      audit.auditSourceIdentification =
        result.auditMessage.auditSourceIdentification
    }
    ...
    if (result.auditMessage.participantObjectIdentification) {
      // xml2js will only use an array if multiple items exist (explicitArray: false), else it's an object
      if (
        result.auditMessage.participantObjectIdentification instanceof Array
      ) {
        for (const poi of Array.from(
          result.auditMessage.participantObjectIdentification
        )) {
          audit.participantObjectIdentification.push(poi)
        }
      } else {
        audit.participantObjectIdentification.push(
          result.auditMessage.participantObjectIdentification
        )
      }
    }
...
export function processAuditMeta(audit, callback) {
  AuditMetaModel.findOne({}, (err, auditMeta) => {
    ...
    if (!auditMeta) {
      auditMeta = new AuditMetaModel()
    }

    if (
      audit.eventIdentification != null &&
      audit.eventIdentification.eventTypeCode != null &&
      audit.eventIdentification.eventTypeCode.code &&
      !codeInArray(
        audit.eventIdentification.eventTypeCode.code,
        auditMeta.eventType
      )
    ...
    if (
      audit.eventIdentification != null &&
      audit.eventIdentification.eventID != null &&
      audit.eventIdentification.eventID &&
      !codeInArray(audit.eventIdentification.eventID.code, auditMeta.eventID)
    ) {
    ...
    if (audit.activeParticipant) {
      for (const activeParticipant of Array.from(audit.activeParticipant)) {
        if (
          (activeParticipant.roleIDCode != null
            ? activeParticipant.roleIDCode.code
            : undefined) &&
          !codeInArray(
            activeParticipant.roleIDCode.code,
            auditMeta.activeParticipantRoleID
          )
        ) {
    ...
    if (audit.participantObjectIdentification) {
      for (const participantObject of Array.from(
        audit.participantObjectIdentification
      )) {
        if (
          (participantObject.participantObjectIDTypeCode != null
            ? participantObject.participantObjectIDTypeCode.code
            : undefined) &&
          !codeInArray(
            participantObject.participantObjectIDTypeCode.code,
            auditMeta.participantObjectIDTypeCode
          )
        ) {
          auditMeta.participantObjectIDTypeCode.push(
    ...
    if (
      (audit.auditSourceIdentification != null
        ? audit.auditSourceIdentification.auditSourceID
        : undefined) &&
      !Array.from(auditMeta.auditSourceID).includes(
        audit.auditSourceIdentification.auditSourceID
      )
    ) {
      auditMeta.auditSourceID.push(
        audit.auditSourceIdentification.auditSourceID
      )
    }
...
export function processAudit(msg, callback) {
  ...
  parseAuditRecordFromXML(parsedMsg.message, (xmlErr, result) => {
    ...
    return audit.save(saveErr => {
      if (saveErr) {
        logger.error(
          `An error occurred while processing the audit entry: ${saveErr}`
        )
      }
      if (xmlErr) {
        logger.info(
          `Failed to parse message as an AuditMessage XML document: ${xmlErr}`
        )
      }
...
function sendUDPAudit(msg, callback) {
  ...
  client.send(
    ...
    0,
    msg.length,
    config.auditing.auditEvents.port,
    config.auditing.auditEvents.host,
    err => {
      client.close()
      return callback(err)
    }
...
export function sendAuditEvent(msg, callback) {
  ...
  if (
    (config.auditing != null ? config.auditing.auditEvents : undefined) == null
  ) {
    return done(
      new Error(
        'Unable to record audit event: Missing config.auditing.auditEvents'
      )
    )
  }
  ...
  switch (config.auditing.auditEvents.interface) {
    ...
    default:
      return done(
        new Error(
          `Invalid audit event interface '${config.auditing.auditEvents.interface}'`
        )
      )
  }
}
