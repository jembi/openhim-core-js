...
export function* getAllMediators() {
  ...
  try {
    const m = yield MediatorAPI.find().exec();
...
export function* getMediator(mediatorURN) {
  ...
  try {
    const result = yield MediatorAPI.findOne({ urn }).exec();
    if (result === null) {
...
export function* addMediator() {
  ...
  try {
    ...
    const existing = yield MediatorAPI.findOne({ urn: mediator.urn }).exec();
    if (existing != null) {
      if (semver.gt(mediator.version, existing.version)) {
        ...
        yield MediatorAPI.findByIdAndUpdate(existing._id, mediator).exec();
      }
    } else {
      ...
      yield Q.ninvoke(new MediatorAPI(mediator), "save");
    }
    this.status = 201;
...
export function* removeMediator(urn) {
  ...
  try {
    yield MediatorAPI.findOneAndRemove({ urn }).exec();
...
export function* heartbeat(urn) {
  ...
  try {
    const mediator = yield MediatorAPI.findOne({ urn }).exec();
    ...
    if (heartbeat != null) {
      ...
      yield MediatorAPI.findByIdAndUpdate(mediator._id, update).exec();
    }
...
export function* setConfig(urn) {
  ...
  try {
    ...
    yield MediatorAPI.findOneAndUpdate({ urn }, { config: this.request.body, _configModifiedTS: new Date() }).exec();
    return this.status = 200;
  } catch (err) {
...
function saveDefaultChannelConfig(channels) {
  ...
  for (const channel of Array.from(channels)) {
    ...
    promises.push(new ChannelAPI(channel).save());
  }
  return promises;
}
...
export function* loadDefaultChannels(urn) {
  ...
  try {
    const mediator = yield MediatorAPI.findOne({ urn }).lean().exec();
