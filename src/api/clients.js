...
export function* addClient() {
  ...
  if (clientData.clientID) {
    const chResult = yield ChannelAPI.find({ allow: { $in: [clientData.clientID] } }, { name: 1 }).exec();
    const clResult = yield ClientAPI.find({ roles: { $in: [clientData.clientID] } }, { clientID: 1 }).exec();
  ...
  try {
    const client = new ClientAPI(clientData);
...
export function* getClient(clientId, property) {
  ...
  try {
    const result = yield ClientAPI.findById(clientId, projectionRestriction).exec();
    if (result === null) {
...
export function* findClientByDomain(clientDomain) {
  ...
  try {
    const result = yield ClientAPI.findOne({ clientDomain }).exec();
    if (result === null) {
...
export function* updateClient(clientId) {
  ...
  try {
    yield ClientAPI.findByIdAndUpdate(clientId, clientData).exec();
...
export function* removeClient(clientId) {
  ...
  try {
    yield ClientAPI.findByIdAndRemove(clientId).exec();
...
export function* getClients() {
  ...
  try {
    return this.body = yield ClientAPI.find().exec();
  } catch (e) {
