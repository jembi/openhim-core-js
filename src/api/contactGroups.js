...
export function addContactGroup() {
  ...
  let contactGroupData = this.request.body;

  try {
    let contactGroup = new ContactGroup(contactGroupData);
    let result = {}; //TODO:Fix yield Q.ninvoke(contactGroup, 'save')
...
export function getContactGroup(contactGroupId) {
  ...
  try {
    let result = {}; //TODO:Fix yield ContactGroup.findById(contactGroupId).exec()
...
export function removeContactGroup(contactGroupId) {
  ...
  contactGroupId = unescape(contactGroupId);
  try {
    let linkedAlerts = yield Channel.find({
      alerts: {
        $elemMatch: {
          groups: {
            $in: [contactGroupId]
...
export function getContactGroups() {
  ...
  try {
    return this.body = {}; //TODO:Fix yield ContactGroup.find().exec()
  } catch (err) {
