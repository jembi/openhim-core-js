...
export function sendEmail(
  ...
  logger.info(
    `Sending email to '${contactAddress}' using service ${nodemailerConfig.service} - ${fromAddress}`
  )
...
function sendSMS(contactAddress, message, callback) {
  ...
  return callback(
    new Error(`Unknown SMS gateway provider '${config.smsGateway.provider}'`)
  )
}
...
function sendSMSClickatell(contactAddress, message, callback) {
  ...
  return axios(
    `http://api.clickatell.com/http/sendmsg?api_id=${config.smsGateway.config.apiID}&` +
      `user=${config.smsGateway.config.user}&password=${config.smsGateway.config.pass}&` +
      `to=${contactAddress}&text=${escapeSpaces(message)}`
  )
    .then(response => {
