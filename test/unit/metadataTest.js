...
describe('Metadata Functions', () => {
  ...
  describe('.getUniqueIdentifierForCollection', () =>
    it("should return objects with the collection's unique attribute and the respective value", done => {
      let result = metadata.getUniqueIdentifierForCollection('Channels', {
        name: 'channelUID'
      })
      result.should.have.property('name', 'channelUID')

      result = metadata.getUniqueIdentifierForCollection('Clients', {
        clientID: 'clientUID'
      })
      result.should.have.property('clientID', 'clientUID')

      result = metadata.getUniqueIdentifierForCollection('Mediators', {
        urn: 'mediatorUID'
      })
      ...
      result = metadata.getUniqueIdentifierForCollection('ContactGroups', {
        groups: 'cgUID'
      })
  ...
  describe('.buildResponseObject', () =>
    it('build a response object', done => {
      ...
      const result = metadata.buildResponseObject(
        model,
        doc,
        status,
        message,
      ...
      result.should.have.property('record', {
        name: 'Channel1',
        urlPattern: 'test/sample'
      })
