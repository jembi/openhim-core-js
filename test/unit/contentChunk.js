...
describe('contentChunk: ', () => {
  ...
  describe('addBodiesToTransactions()', () => {
    ...
    it('should return the transactions with the body payloads', async () => {
      ...
      const routeRequestBodyId = await testUtils.createGridFSPayload('<HTTP body request>') // request payload
      const routeResponseBodyId = await testUtils.createGridFSPayload('<HTTP body response>') // response payload
      tdOne.routes[0].request.bodyId = routeRequestBodyId
      tdOne.routes[0].response.bodyId = routeResponseBodyId
      const orchRequestBodyId = await testUtils.createGridFSPayload('<HTTP body request>') // request payload
      const orchResponseBodyId = await testUtils.createGridFSPayload('<HTTP body response>') // response payload
      ...
      transactionWithBodies[0].orchestrations[0].request.body.should.eql('<HTTP body request>')
      transactionWithBodies[0].orchestrations[0].response.body.should.eql('<HTTP body response>')
      transactionWithBodies[0].routes[0].request.body.should.eql('<HTTP body request>')
      transactionWithBodies[0].routes[0].response.body.should.eql('<HTTP body response>')
