        ...
        retryTransaction1.save(() =>
          autoRetry.createRerunTask([retryTransaction1.transactionID], err => {
            ...
            TaskModel.find({}, (err, results) => {
              ...
              results[0].transactions[0].tid.should.be.exactly(
                retryTransaction1.transactionID.toString()
              )
            ...
            retryTransaction3.save(() =>
              autoRetry.autoRetryTask(null, () =>
                TaskModel.find({}, (err, results) => {
                  ...
                  tids.should.containEql(
                    retryTransaction1.transactionID.toString()
                  )
                  tids.should.containEql(
                    retryTransaction3.transactionID.toString()
                  )
                  return done()
                })
              )
