...
describe('Rewrite URLs middleware', function() {

  describe('.invertPathTransform', () =>

    it('should invert various path transforms', function() {
  ...
  describe('.fetchRewriteConfig', function() {
    ...
    it('should fetch the rewrite config for the current channel and INCLUDE virtual defaults', function(done) {
      currentChannel.addAutoRewriteRules = true;
      let stub = sinon.stub(utils, 'getAllChannelsInPriorityOrder');
      stub.callsArgWith(0, null, [currentChannel, channel1, channel2]);
      return rewriteUrls.fetchRewriteConfig(currentChannel, 'tls', function(err, rewriteConfig) {
    ...
    return it('should fetch the rewrite config for the current channel and EXCLUDE virtual defaults', function(done) {
  ...
  return describe('.rewriteUrls', function() {
    ...
    it('should rewrite absolute hrefs in JSON', function(done) {
      let stub = sinon.stub(rewriteUrls, 'fetchRewriteConfig');
      ...
      return rewriteUrls.rewriteUrls((JSON.stringify(jsonResponse)), channel, 'tls', function(err, newResponse) {
    ...
    it('should rewrite relative hrefs in JSON', function(done) {
      let stub = sinon.stub(rewriteUrls, 'fetchRewriteConfig');
      stub.callsArgWith(2, null, [{
        fromHost:       'route0.org',
        toHost:         'route0To.org',
...
`;
    ...
    return it('should rewrite hrefs in XML', function(done) {
      let stub = sinon.stub(rewriteUrls, 'fetchRewriteConfig');
      stub.callsArgWith(2, null, [{
        fromHost:       'from.org',
        toHost:         'to.org',
      ...
      return rewriteUrls.rewriteUrls(xmlResponse, channel, 'tls', function(err, newResponse) {
        let doc = new dom().parseFromString(newResponse);
        let href1 = xpath.select('string(//someTags/tag1/@href)', doc);
        let href2 = xpath.select('string(//someTags/tag2/child/@href)', doc);
        let src = xpath.select('string(//someTags/img/@src)', doc);
