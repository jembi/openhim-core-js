...
describe('Transaction Alerts', () => {
  before(async () => {
    ...
    await Promise.all([testGroup1.save(), testGroup2.save()])
  ...
  afterEach(async () => {
    await Promise.all([AlertModel.deleteMany({}), EventModel.deleteMany({})])
  ...
  describe('.findTransactionsMatchingStatus', () => {
    it('should return transactions that match an exact status', async () => {
      ...
      const results = await promisify(alerts.findTransactionsMatchingStatus)(
        testChannel,
        {
          condition: 'status',
          status: '404'
    ...
    it('should return transactions that have a matching status in a route response', done => {
      testTransactions[1].save(err => {
        ...
        alerts.findTransactionsMatchingStatus(
          testChannel,
          {
            condition: 'status',
            status: '404'
  ...
  describe('.findTransactionsMaxRetried', () => {
    it('should not return transactions have not reached max retries', done => {
      testTransactions[8].save(err => {
        ...
        alerts.findTransactionsMaxRetried(
          autoRetryChannel,
          autoRetryChannel.alerts[0],
          dateFrom,
          (err, results) => {
            if (err) {
              return done(err)
          ...
          alerts.findTransactionsMaxRetried(
            ...
            (err, results) => {
              ...
              results[0].transactionID
                .equals(testTransactions[9].transactionID)
                .should.be.true()
              return done()
            }
          )
  ...
  describe('.alertingTask', () => {
    ...
    it('should send alerts to users if an alert for the same day was already attempted but it failed', done => {
      ...
      testTransactions[0].save(err => {
        ...
        alerts.alertingTask(
          buildJobStub(dateFrom),
          mockContactHandler(contactSpy, 'Test Failure'),
          () => {
            contactSpy.calledTwice.should.be.true()
            const secondSpy = sinon.spy()
            alerts.alertingTask(
          ...
          alerts.alertingTask(
            ...
            () => {
              ...
              AlertModel.find({}, (err, results) => {
                ...
                resultChannels.should.not.containEql(
                  disabledChannel._id.toHexString()
                )
                return done()
              })
            }
