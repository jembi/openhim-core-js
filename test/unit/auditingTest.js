...
describe('Auditing', () => {
  ...
  describe('.processAudit', () => {
    ...
    it('should parse audit message and persist it to the database', done =>
      auditing.processAudit(testAuditMessage, () =>
        AuditModel.find({}, (err, audits) => {
          ...
          audits[0].eventIdentification.eventOutcomeIndicator.should.be.equal(
            '0'
          )
          ...
          audits[0].eventIdentification.eventID.displayName.should.be.equal(
            'Query'
          )
          audits[0].eventIdentification.eventID.codeSystemName.should.be.equal(
            'DCM'
          )
          audits[0].eventIdentification.eventTypeCode.code.should.be.equal(
            'ITI-9'
          )
          audits[0].eventIdentification.eventTypeCode.displayName.should.be.equal(
            'PIX Query'
          )
          audits[0].eventIdentification.eventTypeCode.codeSystemName.should.be.equal(
            'IHE Transactions'
          )

          audits[0].activeParticipant.length.should.be.exactly(2)
          audits[0].activeParticipant[0].userID.should.be.equal(
            'openhim-mediator-ohie-xds|openhim'
          )
          audits[0].activeParticipant[0].alternativeUserID.should.be.equal(
            '9293'
          )
          audits[0].activeParticipant[0].userIsRequestor.should.be.equal('true')
          audits[0].activeParticipant[0].networkAccessPointID.should.be.equal(
            '192.168.1.111'
          )
          audits[0].activeParticipant[0].networkAccessPointTypeCode.should.be.equal(
            '2'
          )
          audits[0].activeParticipant[0].roleIDCode.code.should.be.equal(
            '110153'
          )
          audits[0].activeParticipant[0].roleIDCode.displayName.should.be.equal(
            'Source'
          )
          audits[0].activeParticipant[0].roleIDCode.codeSystemName.should.be.equal(
            'DCM'
          )
          audits[0].activeParticipant[1].userID.should.be.equal('pix|pix')
          audits[0].activeParticipant[1].alternativeUserID.should.be.equal(
            '2100'
          )
          audits[0].activeParticipant[1].userIsRequestor.should.be.equal(
            'false'
          )
          audits[0].activeParticipant[1].networkAccessPointID.should.be.equal(
            'localhost'
          )
          audits[0].activeParticipant[1].networkAccessPointTypeCode.should.be.equal(
            '1'
          )
          audits[0].activeParticipant[1].roleIDCode.code.should.be.equal(
            '110152'
          )
          audits[0].activeParticipant[1].roleIDCode.displayName.should.be.equal(
            'Destination'
          )
          audits[0].activeParticipant[1].roleIDCode.codeSystemName.should.be.equal(
            'DCM'
          )

          audits[0].auditSourceIdentification.should.exist
          audits[0].auditSourceIdentification.auditSourceID.should.be.equal(
            'openhim'
          )

          audits[0].participantObjectIdentification.length.should.be.exactly(2)
          audits[0].participantObjectIdentification[0].participantObjectID.should.be.equal(
            'fc133984036647e^^^&1.3.6.1.4.1.21367.2005.13.20.3000&ISO'
          )
          audits[0].participantObjectIdentification[0].participantObjectTypeCode.should.be.equal(
            '1'
          )
          audits[0].participantObjectIdentification[0].participantObjectTypeCodeRole.should.be.equal(
            '1'
          )
          audits[0].participantObjectIdentification[0].participantObjectIDTypeCode.code.should.be.equal(
            '2'
          )
          audits[0].participantObjectIdentification[0].participantObjectIDTypeCode.displayName.should.be.equal(
            'PatientNumber'
          )
          audits[0].participantObjectIdentification[0].participantObjectIDTypeCode.codeSystemName.should.be.equal(
            'RFC-3881'
          )
          audits[0].participantObjectIdentification[1].participantObjectID.should.be.equal(
            'c7bd7244-29bc-4ab5-80ee-74b56eed9db0'
          )
          audits[0].participantObjectIdentification[1].participantObjectTypeCode.should.be.equal(
            '2'
          )
          audits[0].participantObjectIdentification[1].participantObjectTypeCodeRole.should.be.equal(
            '24'
          )
          audits[0].participantObjectIdentification[1].participantObjectIDTypeCode.code.should.be.equal(
            'ITI-9'
          )
          audits[0].participantObjectIdentification[1].participantObjectIDTypeCode.displayName.should.be.equal(
            'PIX Query'
          )
          audits[0].participantObjectIdentification[1].participantObjectIDTypeCode.codeSystemName.should.be.equal(
            'IHE Transactions'
          )
          audits[0].participantObjectIdentification[1].participantObjectQuery.should.be.equal(
            testAuditParticipantQuery
          )
          audits[0].participantObjectIdentification[1].participantObjectDetail
            .should.exist
          audits[0].participantObjectIdentification[1].participantObjectDetail.type.should.be.equal(
            'MSH-10'
          )
          audits[0].participantObjectIdentification[1].participantObjectDetail.value.should.be.equal(
            'YmIwNzNiODUtNTdhOS00MGJhLTkyOTEtMTVkMjExOGQ0OGYz'
          )
    ...
    it('should still persist to the database even if the audit includes a non-xml message', done => {
      const nonXmlAudit =
        '<85>1 2015-03-05T12:52:31.358+02:00 Hanness-MBP.jembi.local java 9293 IHE+RFC-3881 - this is a message?>'
    ...
    it('should still persist to the database even if the audit includes an unexpected type of xml message', done => {
      const nonXmlAudit =
        '<85>1 2015-03-05T12:52:31.358+02:00 Hanness-MBP.jembi.local java 9293 IHE+RFC-3881 - <data>data</data>?>'
    ...
    it('should populate audit meta collection with filter fields', done =>
      auditing.processAudit(testAuditMessage, () =>
        AuditMetaModel.findOne({}, (err, auditMeta) => {
          ...
          auditMeta.eventType[0].codeSystemName.should.be.equal(
            'IHE Transactions'
          )
          ...
          auditMeta.activeParticipantRoleID[0].displayName.should.be.equal(
            'Source'
          )
          auditMeta.activeParticipantRoleID[0].codeSystemName.should.be.equal(
            'DCM'
          )
          auditMeta.activeParticipantRoleID[1].code.should.be.equal('110152')
          auditMeta.activeParticipantRoleID[1].displayName.should.be.equal(
            'Destination'
          )
          auditMeta.activeParticipantRoleID[1].codeSystemName.should.be.equal(
            'DCM'
          )
          ...
          auditMeta.participantObjectIDTypeCode[0].displayName.should.be.equal(
            'PatientNumber'
          )
          auditMeta.participantObjectIDTypeCode[0].codeSystemName.should.be.equal(
            'RFC-3881'
          )
          auditMeta.participantObjectIDTypeCode[1].code.should.be.equal('ITI-9')
          auditMeta.participantObjectIDTypeCode[1].displayName.should.be.equal(
            'PIX Query'
          )
          auditMeta.participantObjectIDTypeCode[1].codeSystemName.should.be.equal(
            'IHE Transactions'
          )
  ...
  describe('IHE Samples', () => {
    const validateIHEAudit = function (type, audit) {
      ...
      audit.eventIdentification.eventID.displayName.should.be.equal(
        'UserAuthenticated'
      )
      ...
      audit.eventIdentification.eventTypeCode.displayName.should.be.equal(
        'Login'
      )
      audit.eventIdentification.eventTypeCode.codeSystemName.should.be.equal(
        'DCM'
      )

      audit.activeParticipant.length.should.be.exactly(2)
      audit.activeParticipant[0].userID.should.be.equal(
        'fe80::5999:d1ef:63de:a8bb%11'
      )
      audit.activeParticipant[0].userIsRequestor.should.be.equal('true')
      audit.activeParticipant[0].networkAccessPointID.should.be.equal(
        '125.20.175.12'
      )
      ...
      audit.activeParticipant[0].roleIDCode.displayName.should.be.equal(
        'Application'
      )
      audit.activeParticipant[0].roleIDCode.codeSystemName.should.be.equal(
        'DCM'
      )
      ...
      audit.auditSourceIdentification.auditSourceID.should.be.equal(
        'farley.granger@wb.com'
      )
      return audit.auditSourceIdentification.auditEnterpriseSiteID.should.be.equal(
        'End User'
      )
    }
