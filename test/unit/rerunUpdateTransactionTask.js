...
const ctx3 = {parentID: '53e096fea0af310568333333'}

const ctx4 = {parentID: '53e096fea0af310568444444'}
...
const task1 = new TaskModel({
  ...
  status: 'Processing',
  transactions: [
    {tid: '53e096fea0af3105689acd6a', tstatus: 'Completed'},
  ...
  describe('updateOriginalTransaction', () => {
    it('should update the original transaction with the child ID', done => {
      ...
      TransactionModel.findOne({_id: transactionID}, (err, transaction) => {
        ...
        transaction.should.have.property(
          '_id',
          ObjectId('53e096fea0af3105689acd6a')
        )
        transaction.should.have.property(
          'channelID',
          ObjectId('53bbe25485e66d8e5daad4a2')
        )
        transaction.should.have.property(
          'clientID',
          ObjectId('42bbe25485e77d8e5daad4b4')
        )
        transaction.should.have.property('status', 'Completed')
        transaction.childIDs.length.should.be.eql(0)

        rerunUpdateTransactionTask.updateOriginalTransaction(
          ctx,
          (err, transaction) => {
            if (err) {
              return done(err)
            ...
            transaction.childIDs[0].should.be.eql(
              ObjectId('53e34b955d0180cf6eef2d03')
            )
            return done()
          }
        )
    ...
    it('should update the original transaction with the child ID even when there are orchestrations without a request property', done => {
      ...
      TransactionModel.findOne({_id: transactionID}, (err, transaction) => {
        ...
        transaction.should.have.property(
          '_id',
          ObjectId('53e096fea0af3105689acd6b')
        )
        transaction.should.have.property(
          'channelID',
          ObjectId('53bbe25485e66d8e5daad4a2')
