...
describe('API Integration Tests', () =>
  ...
  describe('Keystore API Tests', function() {
    ...
    it("Should add a new server certificate", done =>
      testUtils.setupTestKeystore(function(keystore) {
        let postData = { cert: fs.readFileSync('test/resources/server-tls/cert.pem').toString() };
    ...
    it("Should return a 400 if the server certificate isn't valid", done =>
      testUtils.setupTestKeystore(function(keystore) {
        let postData = { cert: 'junkjunkjunk' };
        return request("https://localhost:8080")
          .post("/keystore/cert")
          .set("auth-username", testUtils.rootUser.email)
          .set("auth-ts", authDetails.authTS)
    ...
    it("Should add a new server key", done =>
      testUtils.setupTestKeystore(function(keystore) {
        let postData = { key: fs.readFileSync('test/resources/server-tls/key.pem').toString() };
        return request("https://localhost:8080")
          .post("/keystore/key")
          .set("auth-username", testUtils.rootUser.email)
          .set("auth-ts", authDetails.authTS)
    ...
    it("Should add a new trusted certificate", done =>
      testUtils.setupTestKeystore(function(keystore) {
        let postData = { cert: fs.readFileSync('test/resources/trust-tls/cert1.pem').toString() };
        return request("https://localhost:8080")
    ...
    it("Should add each certificate in a certificate chain", done =>
      testUtils.setupTestKeystore(function(keystore) {
        let postData = { cert: fs.readFileSync('test/resources/chain.pem').toString() };
        return request("https://localhost:8080")
          .post("/keystore/ca/cert")
          .set("auth-username", testUtils.rootUser.email)
          .set("auth-ts", authDetails.authTS)
    ...
    it("Should return 400 with there is an invlaid cert in the chain", done =>
      testUtils.setupTestKeystore(function(keystore) {
        let postData = { cert: fs.readFileSync('test/resources/invalid-chain.pem').toString() };
        return request("https://localhost:8080")
          .post("/keystore/ca/cert")
          .set("auth-username", testUtils.rootUser.email)
          .set("auth-ts", authDetails.authTS)
