  ...
  describe('Transactions REST Api testing', () => {
    ...
    describe('*updateTransaction()', () => {
      ...
      it('should update transaction with large routes orchestrations request body', async () => {
        ...
        ;(
          updatedTrans.routes[1].orchestrations[0].request.bodyId !==
          requestBodyId
        ).should.be.true()
        ;(
          updatedTrans.routes[1].orchestrations[0].response.bodyId !==
          responseBodyId
        ).should.be.true()
        updatedTrans.canRerun.should.be.true()
      })
      ...
      it('should not queue a transaction for auto retry when max retries have been reached', async () => {
        ...
        const newTransactionData = Object.assign({}, td, {
          autoRetryAttempt: 5,
      ...
      it('should generate events on update', async () => {
        ...
        td.request.bodyId = requestBodyId
        td.response.bodyId = responseBodyId
        td.orchestrations[0].request.bodyId =
          await testUtils.createGridFSPayload(
            '<HTTP orchestration body request>'
          )
        td.orchestrations[0].response.bodyId =
          await testUtils.createGridFSPayload(
      ...
      it('should update only the relavant supplied orchestration bodies', async () => {
        ...
        const orchestrationRequestBodyId = await testUtils.createGridFSPayload(
          '<HTTP body request orchestration>'
        ) // request payload
        const orchestrationResponseBodyId = await testUtils.createGridFSPayload(
          '<HTTP body response orchestration>'
        ) // response payload
        ...
        ObjectId.isValid(
          updatedTrans.orchestrations[0].request.bodyId
        ).should.be.true()
        ObjectId.isValid(
          updatedTrans.orchestrations[0].response.bodyId
        ).should.be.true()

        // The bodyId shouldnt be the same as the update created new bodyIds
        updatedTrans.orchestrations[0].request.bodyId.should.not.deepEqual(
          orchestrationRequestBodyId
        )
        updatedTrans.orchestrations[0].response.bodyId.should.not.deepEqual(
          orchestrationResponseBodyId
        )
      })
    })
    ...
    describe('*removeTransaction (transactionId)', () => {
      it('should call removeTransaction', async () => {
        ...
        td.routes[0].request.bodyId = await testUtils.createGridFSPayload(
          '<HTTP route body request>'
        )
        td.routes[0].response.bodyId = await testUtils.createGridFSPayload(
          '<HTTP route body response>'
        )
        td.routes[0].orchestrations[0].request.bodyId =
          await testUtils.createGridFSPayload(
            '<HTTP route orchestration body request>'
          )
        td.routes[0].orchestrations[0].response.bodyId =
          await testUtils.createGridFSPayload(
            '<HTTP route orchestration body response>'
          )

        const tx = await new TransactionModel(
          Object.assign({}, td, {clientID: '222222222222222222222222'})
        ).save()
