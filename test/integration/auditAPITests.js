...
describe("API Integration Tests", function() {
  ...
  return describe("Audits REST Api testing", function() {
        ...
        request("https://localhost:8080")
          ...
          .end(function(err, res) {
            if (err) {
              ...
              return Audit.findOne({ "eventIdentification.eventDateTime": "2015-02-20T15:38:25.282Z" }, function(error, newAudit) {
    ...
    describe("*getAudits()", function() {
      ...
      it("should call getAudits ", done =>
        Audit.count({}, function(err, countBefore) {
          let newAudit = new Audit(auditData);
          return newAudit.save(function(error, result) {
            should.not.exist((error));
      ...
      it("should call getAudits with filter paramaters ", function(done) {
        let startDate = "2015-02-20T00:00:00.000Z";
        let endDate = "2015-02-21T00:00:00.000Z";

        let filters = {};
        filters["eventIdentification.eventDateTime"] = "{ \"$gte\": \"2015-02-20T00:00:00.000Z\",\"$lte\": \"2015-02-21T00:00:00.000Z\" }";
        filters = JSON.stringify(filters);

        return Audit.count({}, function(err, countBefore) {
          let audit = new Audit(auditData);
          return audit.save(function(error, result) {
            should.not.exist((error));
            return request("https://localhost:8080")
      ...
      it("should generate an 'audit log used' audit when using non-basic representation", function(done) {
        let audit = new Audit(auditData);
        return audit.save(function(err, result){
          ...
          return request("https://localhost:8080")
            ...
            .end(function(err, res) {
              if (err) {
                ...
                return setTimeout(() =>
                  Audit.find({}, function(err, newAudits) {
                    if (err) { return done(err); }
      ...
      return it("should NOT generate an 'audit log used' audit when using basic (default) representation", function(done) {
        let audit = new Audit(auditData);
        return audit.save(function(err, result){
          if (err) { return done(err); }
          ...
          return request("https://localhost:8080")
            ...
            .end(function(err, res) {
              if (err) {
                ...
                return Audit.find({}, function(err, newAudits) {
                  if (err) { return done(err); }
    ...
    describe("*getAuditById (auditId)", function() {

      it("should fetch a audit by ID - admin user", function(done) {
        let audit = new Audit(auditData);
        return audit.save(function(err, result){
          should.not.exist(err);
          let auditId = result._id;
          return request("https://localhost:8080")
            .get(`/audits/${auditId}`)
            .set("auth-username", testUtils.rootUser.email)
            .set("auth-ts", authDetails.authTS)
      ...
      it("should NOT return a audit that a user is not allowed to view", function(done) {
        let audit = new Audit(auditData);
        return audit.save(function(err, result){
          should.not.exist(err);
          let auditId = result._id;
      ...
      return it("should generate an 'audit log used' audit", function(done) {
        let audit = new Audit(auditData);
        return audit.save(function(err, result){
          if (err) { return done(err); }
    ...
    return describe("*getAuditsFilterOptions", function() {
      ...
      return it("should NOT return a filter dropdown object if user is not admin", function(done) {
        let audit = new Audit(auditData);
        return audit.save(function(err, result){
          should.not.exist(err);
