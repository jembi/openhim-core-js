...
let { Transaction } = require("../../lib/model/transactions");
...
describe("e2e Integration Tests", function() {
  ...
  describe("Authentication and authorisation tests", function() {
    ...
    describe("Mutual TLS", function() {
      ...
      before(function(done) {
        ...
        return channel1.save(function(err) {
          let testClientDoc1 = {
            clientID: "testApp",
          ...
          let client1 = new Client(testClientDoc1);
          let client2 = new Client(testClientDoc2);

          return Client.remove({}, () => client1.save(() => client2.save(() =>
            // remove default keystore
            Keystore.remove({}, function() {
      ...
      it("should forward a request to the configured routes if the client is authenticated and authorised", done =>
        server.start({httpPort: 5001, httpsPort: 5000}, function() {
          let options = {
            host: "localhost",
            path: "/test/mock",
            port: 5000,
      ...
      it("should authenticate a client further up the chain if 'in-chain' config is set", function(done) {
        config.tlsClientLookup.type = "in-chain";
        return server.start({httpPort: 5001, httpsPort: 5000}, function() {
          let options = {
            host: "localhost",
            path: "/test/mock",
            port: 5000,
      ...
      it("should reject a request with an invalid cert if 'in-chain' config is set", function(done) {
        config.tlsClientLookup.type = "in-chain";
        return server.start({httpPort: 5001, httpsPort: 5000}, function() {
          let options = {
            host: "localhost",
      ...
      return it("should NOT authenticate a client further up the chain if 'strict' config is set", function(done) {
    ...
    return describe("Basic Authentication", function() {
  ...
  describe("POST and PUT tests", function() {

    let mockServer = null;
    let mockServerWithReturn = null;
    let testDoc = "<test>test message</test>";
  ...
  describe("HTTP header tests", function() {
  ...
  describe("HTTP body content matching - XML", function() {
...
`;
  ...
  describe("HTTP body content matching - JSON", function() {
...
`;
  ...
  describe("HTTP body content matching - RegEx", function() {

    let mockServer = null;
    let testRegExDoc = "facility: OMRS123";
  ...
  describe("mediator tests", function() {
    ...
    before(function(done) {
      ...
      let mediatorChannel = new Channel({
      ...
      return mediatorChannel.save(function(err) {
        let testAppDoc = {
    ...
    return describe("mediator response processing", function() {
          ...
          request("http://localhost:5001")
            ...
            .end(function(err, res) {
              if (err) {
                ...
                return setTimeout(( () =>
                  Transaction.findOne({}, function(err, res) {
  ...
  describe("Multipart form data tests", function() {
    let mockServer = null;
    ...
    return it("should return 201 CREATED on POST", done =>
      server.start({httpPort: 5001}, function() {
  ...
  describe("URL rewriting e2e test", function() {
  ...
  describe("Routes enabled/disabled tests", function() {
    ...
    before(function(done) {
      ...
      return channel1.save(err => channel2.save(err => channel3.save(function(err) {
        let testAppDoc = {
          clientID: "testApp",
          clientDomain: "test-client.jembi.org",
          name: "TEST Client",
        ...
        request("http://localhost:5001")
          ...
          .end(function(err, res) {
            if (err) {
              ...
              // routes are async
              return setTimeout(( () =>
                Transaction.findOne({}, function(err, trx) {
  ...
  return describe("Channel priority tests", function() {
    ...
    return it("should deny access if multiple channels match but the top priority channel denies access", function(done) {
      let channel4 = new Channel({
