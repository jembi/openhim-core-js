...
describe "e2e Integration Tests", ->
  ...
  describe "Internal server error tests", ->
    ...
    it "should mark transaction as an internal server error", (done) ->
      server.start httpPort: 5001, ->
        request("http://localhost:5001")
          .get("/test/nowhere")
          .auth("testApp", "password")
          ...
          .end (err, res) ->
            if err
              ...
              setTimeout ( ->
                Transaction.findOne {}, (err, trx) ->
                  return done err if err
                  trx.should.have.property 'internalServerError'
                  trx.should.have.property 'internalServerErrorMessage'
                  trx.should.have.property 'internalServerErrorStack'
                  trx.internalServerError.should.be.true()
                  (trx.internalServerErrorMessage.indexOf('ECONNREFUSED') > -1).should.be.true()
                  done()
              ), 150 * global.testTimeoutFactor
